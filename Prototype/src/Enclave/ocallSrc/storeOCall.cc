...
/**
 * @brief persist the buffer to file (with MLE Key version)
 * 
 * @param outClient the out-enclave client ptr
 */
void Ocall_UpdateFileRecipeWithMLEKey(void* outClient) {
    ClientVar* outClientPtr = (ClientVar*)outClient;
    Recipe_t* outRecipe = &outClientPtr->_outRecipe;
    storageCoreObj_->UpdateRecipeToFileWithMLEKey(outRecipe->entryList,
        outRecipe->recipeNum, outClientPtr->_recipeWriteHandler);
    outRecipe->recipeNum = 0;
    return ;
}
...
void Ocall_QueryOutIndex(void* outClient) {
    ...
    tmpChunkAddress.resize(CONTAINER_ID_LENGTH * 2, 0);
    bool queryResult;
    for (size_t i = 0; i < outQuery->queryNum; i++) {
        ...
        if (queryResult) {
            ...
            memcpy(&entry->edgeContainerName, tmpChunkAddress.c_str(), CONTAINER_ID_LENGTH);
            memcpy(&entry->cloudContainerName, tmpChunkAddress.c_str() + CONTAINER_ID_LENGTH, CONTAINER_ID_LENGTH);
        } else {
 ...
 * @param outClient the out-enclave client ptr
 */
void Ocall_QueryOutIndexForRestore(void* outClient) {
#if (MULTI_CLIENT == 1)
    pthread_rwlock_rdlock(&outIdxLck_);
#endif
    ClientVar* outClientPtr = (ClientVar*)outClient;
    size_t recipeNum = outClientPtr->recipeNum;
    OutRestoreEntry_t* OutRestoreEntry = outClientPtr->_outRestoreEntry;

    // printf("recipeNum: %lu\n", recipeNum);
    // for (size_t i = 0; i < CHUNK_HASH_SIZE; i++) {
    //     printf("%d\n", RestoreEntry->chunkHash[i]);
    // }

    string tmpChunkAddress;
    string tmpChunkHashStr;
    bool isSuccess;
    
    for (size_t i = 0; i < recipeNum; i++) {
        // query full FP index
        tmpChunkHashStr.assign((char*)OutRestoreEntry->chunkHash, CHUNK_HASH_SIZE);
        tmpChunkAddress.resize(CONTAINER_ID_LENGTH * 2, 0);
        isSuccess = indexStoreObj_->QueryBuffer(tmpChunkHashStr.c_str(), 
            CHUNK_HASH_SIZE, tmpChunkAddress);

        memcpy(OutRestoreEntry->edgeContainerName, tmpChunkAddress.c_str(), CONTAINER_ID_LENGTH);
 
        // if(isSuccess) printf("check successful\n");
...
void Ocall_UpdateOutIndex(void* outClient) {
    ...
    OutQuery_t* outQuery = &outClientPtr->_outQuery;
    OutQueryEntry_t* entry = outQuery->outQueryBase;
    string edgeContainerName;
    string cloudContainerName;
    char insertValue[CONTAINER_ID_LENGTH * 2];
    
    for (size_t i = 0; i < outQuery->queryNum; i++) {
        // update the outside index
        if (entry->dedupFlag == UNIQUE) {
            edgeContainerName.assign((char*)entry->edgeContainerName, CONTAINER_ID_LENGTH);
            cloudContainerName.assign((char*)entry->cloudContainerName, CONTAINER_ID_LENGTH);

            // printf("edge container name: %s\n", edgeContainerName.c_str());

            memcpy(insertValue, edgeContainerName.c_str(), CONTAINER_ID_LENGTH);
            memcpy(insertValue + CONTAINER_ID_LENGTH, cloudContainerName.c_str(), CONTAINER_ID_LENGTH);
            ...
            indexStoreObj_->InsertBothBuffer((char*)entry->chunkHash, CHUNK_HASH_SIZE,
                insertValue, CONTAINER_ID_LENGTH * 2);
        } 
        entry++;
    }
#if (MULTI_CLIENT == 1)
