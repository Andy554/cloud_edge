...
void EnclaveClient::InitRestoreBuffer() {
    ...
    // for recipe
    _plainHashBuffer = (uint8_t*) malloc(Enclave::sendRecipeBatchSize_ *
        CHUNK_HASH_SIZE);
    _inRestoreEntry = (InRestoreEntry_t*) malloc(Enclave::sendRecipeBatchSize_ *
        sizeof(InRestoreEntry_t));
    return ;
}
...
void EnclaveClient::DestroyRestoreBuffer() {
    free(_plainHashBuffer);
    free(_inRestoreEntry);
